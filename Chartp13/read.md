...
// std::move(x) 返回Foo&&，转换为const Foo&是可行的。
```
...
``` 
不管使用拷贝构造函数还是移动构造函数，赋值运算符函数体都swap两个运算符对象的状态，交换HasPtr会交换两个对象的指针及int成员。

### 建议 三五法则
所有五个拷贝控制成员应该看做一个整体：一般来说，如果一个类定义了任何一个拷贝操作，他就应该定义所有五个操作。某些类碧玺定义拷贝构造函数、拷贝赋值运算符和析构函数才能正确工作。

#### 右值引用和成员函数

除了构造函数和赋值运算符之外，如果一个成员函数同时提供拷贝和移动版本，他也能从中受益。
拷贝或者移动构造函数和赋值运算符相同的参数模式：一个版本接受一个指向const的左值引用，第二个版本接受一个指向非const的右值引用。
```c++
void push_back(const X&); // 拷贝：绑定到任意类型的X
void push_back(X&&); 	  // 移动：只能绑定到类型X的可修改的右值。
```
一般来说，我们不需要为函数操作定义接受一个`cosnt X&&`或者`X&`参数的版本。
...
auto n = (s1 + s2).find('a');
```
在旧标准中，我们没有办法阻止这种使用方式。为了维持向后兼容性，新标准库仍然允许右值赋值。
